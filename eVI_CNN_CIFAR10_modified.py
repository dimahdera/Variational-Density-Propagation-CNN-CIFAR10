...
from Covariance_propagation import first_convolution,intermediate_convolution, intermediate_convolution_approx, activation, max_pooling, fully_connected
...
def Model_with_uncertainty_computation(x, y_label,w_mean, s, new_size,  new_size2, new_size3, keep_prob1, keep_prob2, keep_prob3,
    ...
    sigma_z2 = intermediate_convolution_approx(w_mean['m2'],s['s2'],mu_z2,sigma_g,patch_size,num_filters[0],num_filters[1], image_size, new_im_size, pad="VALID")
    image_size = image_size - patch_size + 1
    ######################################################         
    ...
    sigma_z3 = intermediate_convolution_approx(w_mean['m3'],s['s3'],mu_z3,sigma_p,patch_size,num_filters[1],num_filters[2], new_size, new_im_size, pad="VALID")
    ...
    sigma_z4 = intermediate_convolution_approx(w_mean['m4'],s['s4'],mu_z4,sigma_g3,patch_size,num_filters[2],num_filters[3], new_size, new_size, pad="SAME")
    ...
    sigma_z5 = intermediate_convolution_approx(w_mean['m5'],s['s5'],mu_z5,sigma_p2,patch_size,num_filters[3],num_filters[4], new_size2, new_size2, pad="SAME")
    ...
    sigma_z6 = intermediate_convolution_approx(w_mean['m6'],s['s6'],mu_z6,sigma_g5,patch_size,num_filters[4],num_filters[5], new_size2, new_size2, pad="SAME")
...
def main_function(image_size=32, num_channel=3, patch_size=3, num_filter=[32, 32, 64,64,128,128],num_labels=10,
        ...
        weight_std=0.05, epochs=40, Adversarial_noise=False, Random_noise=False, gaussain_noise_var=0.01, Training=True, continue_train=False):     
    init_std = 0.1
    ...
    ######################################################          
    output_sigma1 = tf.clip_by_value(t=(output_sigma) , clip_value_min=tf.constant(1e-7), 
    ...
    if os.path.exists('./EVI_CIFAR10_with_sigma_{}/epochs_{}/model.meta'.format(init_std, epochs)):     

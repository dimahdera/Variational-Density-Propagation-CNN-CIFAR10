...
def intermediate_convolution_approx(w_mean, w_s, mu_z, pre_sigma, patch_size , num_filters0, num_filters1, im_size, new_im_size, pad="VALID"):
    ...
    diag_sigma_g = tf.reshape(diag_sigma_patches, [-1, patch_size*patch_size*num_filters0] )
    mu_cov = tf.reshape(w_mean, [patch_size*patch_size*num_filters0, num_filters1])
    mu_cov_squar = tf.multiply(mu_cov, mu_cov)
    sigma_z_ = tf.transpose(tf.matmul(diag_sigma_g,mu_cov_squar))#shape=[num_filters1,new_im_size*new_im_size]     
    mu_h_sigma_mu_h = tf.matrix_diag(sigma_z_) #shape=[num_filters1, new_im_size*new_im_size, new_im_size*new_im_size]
